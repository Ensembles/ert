...
static int lsf_driver_submit_shell_job(lsf_driver_type * driver ,
...

  {
    ...
    if (driver->submit_method == LSF_SUBMIT_REMOTE_SHELL) {
      ...
      util_spawn(driver->rsh_cmd, 2, (const char **) argv, tmp_file, NULL, NULL);
      ...
      util_spawn(driver->bsub_cmd, stringlist_get_size( remote_argv), (const char **) argv, tmp_file, tmp_file, NULL);
      free( argv );
    }
...
static void lsf_driver_update_bjobs_table(lsf_driver_type * driver) {
  ...
  if (driver->submit_method == LSF_SUBMIT_REMOTE_SHELL) {
    ...
    argv[0] = "-a";
    util_spawn(driver->bjobs_cmd, 1, (const char **) argv, tmp_file, NULL, NULL);
    free( argv );
  }
  ...
  lsf_job_type    * job    = lsf_job_safe_cast( __job );
  {
    ...
    lsf_driver_internal_error( driver );
      ...
      if (driver->submit_method == LSF_SUBMIT_REMOTE_SHELL) {
        ...
        util_spawn(driver->rsh_cmd, 2, (const char **) argv, NULL, NULL, NULL);

        free( argv[1] );
        free( argv );
      } else if (driver->submit_method == LSF_SUBMIT_LOCAL_SHELL)
        util_spawn(driver->bkill_cmd, 1, (const char **) &job->lsf_jobnr_char, NULL, NULL, NULL);
    }
  }
}
