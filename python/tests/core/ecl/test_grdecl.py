...
class GRDECLTest(ExtendedTestCase):
    ...
    def test_Load( self ):
        kw = EclKW.read_grdecl(Stream(self.src_file, "r"), "PERMX")
        self.assertTrue(kw)

        grid = EclGrid( self.createTestPath("Statoil/ECLIPSE/Gurbat/ECLIPSE" ))
        kw = Ecl3DKW.read_grdecl(grid , Stream(self.src_file, "r"), "PERMX")
    ...
    def test_reload( self ):
        ...
        fileH = Stream(tmp_file1, "w")
        kw.write_grdecl(fileH)
        fileH.close()

        kw1 = EclKW.read_grdecl(Stream(tmp_file1, "r"), "PERMX")

        fileH = Stream(tmp_file2, "w")
